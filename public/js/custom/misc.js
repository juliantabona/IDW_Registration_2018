(function($) {
  ...
  $(function() {
    ...
    $('.nav li a', sidebar).each(function(){
        ...
        if(current === "") {
          ...
          if($this.attr('href').indexOf("index.html") !== -1){
              $(this).parents('.nav-item').last().addClass('btn-dark').children().children().removeClass('text-dark').addClass('text-white');
          ...
          //for other url
          if($this.attr('href').split("/").slice(-1)[0].indexOf(current) !== -1){
          
              $(this).parents('.nav-item:not(.nav-unhighlightable)').last().addClass('btn-dark').children('a').children().removeClass('text-dark').addClass('text-white');

              if ($(this).parents('.sub-menu').length) {
                $(this).parents('.sub-menu').addClass('bg-white p-0 pl-4').children('li').children('a').addClass('pl-4');
                $(this).closest('.collapse').addClass('show');
    ...
    $('button[type="submit"], .loadable-btn, .swal-button').click(function(){
      var button = $(this);
...
function makeSubmitableBtn(btn){
  var button = $(btn);
  if(button.find('.loader').length == 0){
      button.find('i').hide();  
      button.find('span').text( button.find('span').attr('app-load') ); 
